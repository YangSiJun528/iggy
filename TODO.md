...
(왜냐면 식별 코드가 없어서 payload 형식을 뭔지 모르기 때문이였음.)
...
1. spec 문서 만들기
   - `core/server/src/binary/command.rs`랑 BytesSerializable 구현체 보고 가능한 페이로드 값들 목록 구하기
     - [trait.BytesSerializable.html](target/doc/iggy_common/trait.BytesSerializable.html)
   - 요청/응답 페이로드 더 정확히 찾아보기
     - 응답 페이로드도 BytesSerializable를 사용하는지? 그러면 구분해서 명세하고, 가능한 범위를 문서화하기
       - 어떤 코드를 참고했는지도 함께 정리.
2. lua 아키텍처 만들기
   - 일단 어느정도 잘 만들어져서 그거 쓰면 될 듯?
   - 대신 하나씩 만든다는 느낌으로 직접 짜보고
   - 포트 번호로 인식하는거는 AI한테 데모 1~2번은 더 뽑아봐야 할 듯?
3. Rust 테스트 작성하기
   - 이게 Rust 직렬화된 데이터로 검사한다는거에서 의의가 있긴한데, 그냥 파일로 저장하는 패치 만들어서 쉘같은걸로 검사하는게 더 나을수도 있음.
   - 일단 초안 작성해두고 피드백 받기
4. 초안 피드백 받기
...
불필요하다고 생각된 부분 정리함.
이제 슬슬 이해가는 부분 많아짐. 이해 안가던 부분이 대부분이 불필요한 부분이였음.

코드 마지막으로 분석하고 이해 안가는 부분 찾아보고, 필드 관리를 어떻게 할지는 생각해봐야 할 듯?
공통 영역을 관리하는것도 좋인데, 그럼 전역으로 관리하는게 자연스럽고, 이러면 수백개를 관리해야 한다는 입장 하에 찾아보기 좀 불편할수도?
commands 안에 넣으면 결합도도 좋고 패킷 순서를 바로 볼 수 있다는 장점이 있는데, 중복이 생김.
- 근데 중복 문제는 애초에 공통 컬럼을 두면 그 때부터 생기는 문제임.
- 전역/commands 별 과 별개로 공통 컬럼을 어떻게 할지부터 생각해봐야 함.
  - 그게 전역/commands 별 관리 방법에도 영향을 줄거임.

일단 다 정리하고, 2가지 옵션 만들어서 디스코드에 올려보는 것도 좋을지도?
아니면 디스코드에 올릴지 깃헙에 올릴지도 물어보고 해도 괜찮고

-----

이제 진짜 lua 구현은 다 된듯?
quic 조건 붙이는 부분 추가하기
생각해보니 이러면 pref 포트가 2개 필요하겠네 (각자 큐 2개 있어도 문제될 건 없어보임.)
이거만 추가하고 rust로 테스트 간단하게 하기...

rust 구현 먼저 대충 이해해보고 적당히 손보기, lua 주석 불필요한거 다 지우고 위치 옮기고
rust는 일단 실제 로직으로 테스트해본다는 느낌이긴 한데, 가능하면 좀 흠... 단위 테스트 느낌으로 하면 좋을거 같긴 함.
그냥 to_bytes() 결과 패킷으로 쓰고 그거 역분석하고, 아니면 pnanp? 확장자 파일로 단위테스트도 되긴 한데, 이거는 좀... 깨진 상태로 계속 유지될까봐 그렇긴 함.
가능하면 변하는 Rust 코드에 따라 영향을 받는 테스트로 설계하기.  

lua 구현 관련해서는 경험치 버리기 아까우니 정리 좀 해서 남기기

PR 열어서 피드백 받기

-------

QUIC는 보류, 전송 데이터가 암호화(TLS) 되서, 이 때 쓰는 키 로그 파일을 받아야 함.   
뭐 어떻게 하면 가능할거 같긴 한데, 잘 모르겠음.

이러면 Rust 테스트 코드 분석부터 하고? 
lua 구현 마지막으로 정리하면서 공부하고 + 다듬기

-------

러스트 코드 정리도 좀 함.

일단 tshark나 초기 세팅하는 부분 빼고 assert 쪽만 정리함. 그래도 꽤 괜찮아짐.

초기 세팅 부분 보고 정리하면 될 듯?
